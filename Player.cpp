...
Player::Player(float w, float h, float x, float y, float t, float intertia_damp) {
	shader = nullptr;
	m_vaoID = 0;
	m_vboID[0] = 0;
	m_vboID[1] = 0;
	...
	thrust = t;
	interia_dampners = intertia_damp;
	angle = 0.0f;
	mass = 1;
	health = 3;
...
void Player::init(float colour[3]){
...
void Player::render(glm::mat4& ModelViewMatrix, glm::mat4& ProjectionMatrix){
